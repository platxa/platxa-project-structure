...
| `--seed-memory` | Seed a starter `MEMORY.md` into the machine-local auto-memory dir — see [Seed Auto Memory](#seed-auto-memory) |
...
Invoked via the opt-in `--seed-memory` flag. Writes a starter `MEMORY.md` index into the machine-local auto-memory directory so the user understands that auto memory coexists with the generated rules.

**Canonical location** (per <https://code.claude.com/docs/en/memory#storage-location>): the project's auto-memory directory is derived from the git repo root. Typically:

```
~/.claude/projects/<sanitized-repo-path>/memory/MEMORY.md
```

The exact directory is managed by Claude Code itself — do NOT attempt to compute the path. Instead, the `--seed-memory` flag prints instructions for the user to run `/memory` (which opens the auto-memory folder) and provides the content to paste.

### Behavior

1. **Check**: is auto memory enabled? Run `claude config get autoMemoryEnabled` if available, or default to assuming enabled (it's the default). If disabled, print a note and skip.
2. **Print a starter `MEMORY.md` template** to the terminal for the user to paste into their auto-memory folder:

   ```markdown
   # MEMORY.md — Auto memory index for {{project_name}}

   This file is the index for machine-local auto memory. Claude writes detailed
   notes into topic files (debugging.md, patterns.md, etc.) and uses this index
   to keep track of what's stored where.

   See https://code.claude.com/docs/en/memory#auto-memory for how auto memory
   works. This file is machine-local — it is NOT committed and is NOT shared
   with the team. Team-shared guidance belongs in ./CLAUDE.md (generated by
   /platxa-project-structure:setup).

   ## Known topic files

   (none yet — Claude will populate this as it learns about the project)
...
3. **Print instructions** for the user:
   ...
   ```
   To seed your auto memory, open the folder with:
     /memory

   then paste the template above into MEMORY.md (create it if missing).
   ```

4. **NEVER write to `~/.claude/projects/` directly** — the path is Claude-managed, and writing to it from a project context would conflict with Claude Code's own management. The flag is advisory-only: it prints content for the user to manually seed.
